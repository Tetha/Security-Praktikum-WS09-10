...
\phase{Phase 2.2: Bestimmung eines Schwellwertes, der Spam, Nicht-Spam Anteile m\"oglichst gut trennt};
...
ist in Abbildung \ref{fig:yao} skizziert und basiert auf \cite{yaoprotocol}.
...
der beiden Anwender, ohne Beeintr\"achtigung der Allgemeinheit Alice, muss
...
Wenn wir zudem definieren, dass die Ausgabe 0 bedeutet, dass die erste
 ...
 angedeutet. Ein Automat zum Vergleichen ist dann in \ref{compare_automata} 
 genau implementiert. \\
\begin{figure}
\centering
\begin{tikzpicture}
...
Wir implementieren diesen Schaltkreis als Zustandsmaschine, die die
internen Zust\"ande ,,Keine Mails'' ,,Alles Spam'', ,,Kein Spam''
und ,,beide Labels'' hat.  Der Zustands\"ubergangsgraph des Automaten
ist in \ref{fig:states:uniqueness} dargestellt. \begin{figure}
\begin{tikzpicture}[text width = 2cm]
\node[state, initial] (nomails) {Keine Mails};
\node[state] (allspam) [above right=of nomails] {Alles Spam};
\node[state] (allgood) [below right=of nomails] {Kein Spam};
\node[state] (both) [below right=of allspam] {Beide Labels};

\draw[->] (nomails) edge node [right] {Spam} (allspam)
                    edge node [left] {kein Spam} (allgood)
                    edge [loop above] node [left] {Abwesenheit} (nomails);
\draw[->] (allspam) edge [loop above] node {Spam, Abwesenheit} (allspam)
                    edge node [right]{kein Spam} (both);
\draw[->] (allgood) edge [loop below] node {kein Spam, Abwesenheit} (allgood)
                    edge node [right] {Spam} (both);
\draw[->] (both) edge [loop right] node {*} (both);
\end{tikzpicture}
\caption{Zustands\"ubergangsgraph des Eindeutigkeitsautomaten}
\label{fig:states:uniqueness}
\end{figure}
...
\begin{figure}
\begin{tikzpicture}
\node[state,initial] (unentschieden) {\(a = b\)};
\node[state] (less) [below left=of unentschieden] {\(a < b\)};
\node[state] (more) [below right=of unentschieden] {\(a > b\)};

\draw[->] (unentschieden) edge [loop above] node {\(a_i,b_i = (1,1),(0,0)\)} (unentschieden)
                          edge              node [left] {\(a_i,b_i = (0,1)\)} (less)
                          edge              node [right] {\(a_i,b_i = (1,0)\)} (more);

\draw[->] (less) edge [loop below] node { * } (less);
\draw[->] (more) edge [loop below] node { * } (more);
